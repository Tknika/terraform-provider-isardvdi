...
data "isard_users" "search_by_name" {
  name_filter = "admin"
}

output "users_by_name" {
  description = "Usuarios que contienen 'admin' en su nombre"
  value = [for user in data.isard_users.search_by_name.users : {
    id       = user.id
    name     = user.name
...
output "category_users_count" {
  ...
  value       = length(data.isard_users.category_users.users)
}
...
output "manager_list" {
  ...
  value = [for user in data.isard_users.managers.users : {
    name         = user.name
    email        = user.email
...
data "isard_users" "specific_user" {
  name_filter = var.search_username
}

output "specific_user_details" {
  description = "Detalles del usuario buscado"
  value = length(data.isard_users.specific_user.users) > 0 ? {
    id                     = data.isard_users.specific_user.users[0].id
    name                   = data.isard_users.specific_user.users[0].name
    username               = data.isard_users.specific_user.users[0].username
    email                  = data.isard_users.specific_user.users[0].email
    role                   = data.isard_users.specific_user.users[0].role_name
    category               = data.isard_users.specific_user.users[0].category_name
    group                  = data.isard_users.specific_user.users[0].group_name
    active                 = data.isard_users.specific_user.users[0].active
    email_verified         = data.isard_users.specific_user.users[0].email_verified
  } : null
}
...
output "active_users_in_group" {
  ...
  value = [for user in data.isard_users.active_group_users.users : {
    name     = user.name
    username = user.username
    role     = user.role_name
  }]
...
data "isard_users" "filtered_users" {
  category_id = "default"
  role        = "user"
  active      = true
}
...
output "filtered_users_summary" {
  ...
  value = {
    total_count = length(data.isard_users.filtered_users.users)
    users       = [for user in data.isard_users.filtered_users.users : user.username]
  }
}
...
output "all_user_ids" {
  ...
  value = [for user in data.isard_users.search_by_name.users : user.id]
}
